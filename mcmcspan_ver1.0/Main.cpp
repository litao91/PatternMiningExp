...
int main(int argc, char **argv)
{
    ...
    prefixspan.print_closed_pattern();
    return 0;
}
...
void Version(){
    /*
    printf("This version is modified by Gary LI. Tao, sept. 23 th\n");
    printf("Add closed frequent and maximum pattern\n");
    printf("The original version can be found in http://code.google.com/p/prefixspan/\n");
    */
}
    ...
    for (int argno = 1; argno < argc; argno++){
        if (argv[argno][0] == '-'){
            if(!strcmp (argv[argno], "-version")){
                ...
                if (argno == argc - 1){
                    cerr << "Must specify minimum support after -min_sup" << endl;
                }
                min_sup = atoi(argv[++argno]);
            }
            else if (!strcmp (argv[argno], "-max_pat")) {
                if (argno == argc - 1) {
                    cerr << "Must specify miximum itemset size after -max_size" << endl;
                }
